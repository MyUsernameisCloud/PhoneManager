...
import com.yun.phonemanager.dao.BlackNumberDao;
import com.yun.phonemanager.domain.BlackNumberInfo;
...
public class CallSafeActivity extends BaseTouch2 {
    ...
    private List<BlackNumberInfo> infos;// 存放所有黑名单信息
    ...
    public void showDialog() {
       // AlertDialog dialog;
        ...
        // 填充自定义的View
        View view = View.inflate(CallSafeActivity.this, R.layout.dialog_blacknumber, null);
        // 找到自定义View中的组件冰给确定和取消键设置监听
        final EditText et_phone = (EditText) view.findViewById(R.id.et_black_dialog);
            ...
            switch (scrollState) {
                case OnScrollListener.SCROLL_STATE_IDLE:// 禁止空闲状态
                    if (lv_blackNumber.getLastVisiblePosition() == infos.size() - 1) {
                        if (isLoading) {
                            Toast.makeText(CallSafeActivity.this, "已经在加载数据了", Toast.LENGTH_SHORT)
                                    .show();
                            return;
                        }
                        if (index >= total) {
                            Toast.makeText(CallSafeActivity.this, "已经没有更多数据了",Toast.LENGTH_SHORT)
                                    .show();
                            return;
                        }
                        isLoading = true;
    ...
    class BlackNumberAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, ViewGroup parent){
            ...
            if (convertView != null) {
                ...
                holder.iv_black_delete = (ImageView) view.findViewById(R.id.iv_black_delete);
