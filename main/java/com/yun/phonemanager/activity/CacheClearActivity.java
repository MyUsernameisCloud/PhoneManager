...
public class CacheClearActivity extends BaseTouch2 {
    ...
    class MyAdapter extends BaseExpandableListAdapter {
        ...
        public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
            ...
            //第一项
            if (groupPosition == 0) {
                view = View.inflate(CacheClearActivity.this, R.layout.item_cache_group, null);
                TextView tv = (TextView) view.findViewById(R.id.tv_cache_grouptotal);
                tv.setText("内部缓存");
				/*
				formatFileSize()叫你传入一个long类型的值，首先它怎么知道这是B、KB、MB、G呢？
				它会以为这是B(1KB=1024B),然后除以1024，单位就是KB，看是否大于900，
				若大于，就在除以1024，单位就是MB，然后继续判断
				*/
        ...
        //ExpandableListView展开后的内容
        public View getChildView(int groupPosition, final int childPosition,
                                 boolean isLastChild, View convertView, ViewGroup parent) {
            View v = null;
            if (groupPosition == 0) {
                ViewHolder holder = null;
                if (convertView != null) {  //这样做是为了节约系统资源
                    v = convertView;
                    holder = (ViewHolder) v.getTag();
                } else {
                    v = View.inflate(CacheClearActivity.this, R.layout.item_cache_child, null);
                    holder = new ViewHolder();
                    holder.iv_icon = (ImageView) v.findViewById(R.id.iv_icon);
                    holder.iv_cache_delete = (ImageView) v.findViewById(R.id.iv_cache_delete);
                    holder.tv_cache_childname = (TextView) v.findViewById(R.id.tv_cache_childname);
                    holder.tv_cache_childtotal = (TextView) v.findViewById(R.id.tv_cache_childtotal);
                    v.setTag(holder);
                }
                holder.iv_icon.setBackgroundDrawable(innerInfos.get(childPosition).getIcon());
                holder.tv_cache_childname.setText(innerInfos.get(childPosition).getName());
                holder.tv_cache_childtotal.setText(Formatter.formatFileSize(
                        CacheClearActivity.this, innerInfos.get(childPosition).getCache()));
                holder.iv_cache_delete.setOnClickListener(new OnClickListener() {
                            public void onClick(View v) {
                                // TODO Auto-generated method stub
                                try {
                                    //通过反射调用PackageManager雷利的deleteApplicationCacheFiles方法
                                    //PackageManager.class 返回一个Class类对象
                                    //如果T是任意的Java类型（或void关键字），
                                    //T.class将代表匹配的类的对象
                                    //Class c1 = PackageManager.class；
                                    //Class类的getdeclareMethods方法将返回类中声明的全部方法
                                    //其中包括私有和受保护成员，但不包括超类的成员
                                    //为了实现调用类中的任意方法，需要用到Method类
                                    ...
                                    method.invoke(pm,
                                            innerInfos.get(childPosition).getPackageName(),
    ...
    Handler handler = new Handler() {
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                ...
                case FINISH:
                    if (cacheTotal > 0) {
                        ll_cache_top.setBackgroundColor(getResources().getColor(R.color.brown));
    ...
    public void check() {
        new Thread() {
            public void run() {
                List<AppInfo> infos = APPInfoProvider.getAppInfo(CacheClearActivity.this);
    ...
    public void OnClickclearAll(View v) {
        //通过反射调用freeStorageAndNotify方法实现一键清理
        try {
            Method method = PackageManager.class.getMethod(
